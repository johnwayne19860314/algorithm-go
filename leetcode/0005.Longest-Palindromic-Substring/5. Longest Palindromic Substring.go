...
func getlonggestPalindromic(input string) (res string) {
	res = ""
...
func getLonggest5(s string) string{
	if len(s) == 1{
		return s
	}
	res := ""
	for i:= 0; i< len(s); i++{
		res = getMax(s,res,i,i)
		res = getMax(s,res,i,i+1)
	}
	return res
}

func getMax(s,res string, i,j int) string{
	tmp := ""
	for i>=0 && j<len(s)&&s[i]==s[j]{
		tmp = s[i:j+1]
		i--
...
func getLongestPandin(s string) string {
	// left right as the moving window; pl,pr as the longest one
	// set right init val as 
	left,right,pl,pr := 0,0,0,0
	for left<len(s) {
		for s[left] == s[right+1]{
			right++
		}
		for left>=1&&right<len(s)-1&&s[left-1] == s[right+1] {
			left--
			right++
		}
		if (right-left) > (pr-pl) {
