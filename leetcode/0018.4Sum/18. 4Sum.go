...
func quadrupSum0413(input []int, target int) [][]int{
	...
	for i:=0; i< len-3 ;i++ {
		// performance improvement; reduce the duplicates
		a := input[i]
		first := strconv.Itoa(a)
		second := strconv.Itoa(input[i+1])
		firstTwo := first
		firstTwo += second
		// it saves the most computation by moving i ahead
		if ok := checker[firstTwo]; ok{
			continue
		}
		
		for j := i+1; j<len-2; j++ {
			b := input[j]
			second = strconv.Itoa(b)
			firstTwo := (first+second)
			if ok := checker[firstTwo]; !ok{
				checker[firstTwo] = true
			}else{
				// it save the computation of l.k loops
				continue
			}
			
			for l,k := j+1,len-1; l<k; {
				// l,k move when dup
				c,d := input[l],input[k]
				sum := (a+b+c+d)
				if sum == target{
					firstThree := (firstTwo+strconv.Itoa(c))
					if ok:= checker[firstThree]; !ok {
						res = append(res, []int{a,b,c,d})
						checker[firstThree] = true
					}
